...
UCHAR MakePalette(UCHAR **ImageBytes,UINT CtoRemove,UINT ICtoRemove)
{
    UINT key,i;
    int y;
...
void SaveImage(UCHAR **ImageBytes,USHORT xx,USHORT yy,UCHAR *name,USHORT type)
{
    ...
    UINT CtoRemove,ICtoRemove;
    if(ISCHECKED(cTransparent)){ CtoRemove=ColorToRemove; }else{ CtoRemove=0xFFFFFFFF; }
    if(ISCHECKED(cITransparent)){ ICtoRemove=IColorToRemove; }else{ ICtoRemove=0xFFFFFFFF; }
    
    if(MakePalette(ImageBytes,CtoRemove,ICtoRemove) == NO)
    {

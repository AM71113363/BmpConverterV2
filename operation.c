...
void LoadIconCursor()
{
     ...
     for(x=0;x<AllPixels;x++)
     {
         if(BGRA[x]==1) //transparent color,
         {                   
            if(t[(x*3)]==0) //transparent BLACK
            {
               t[(x*3)]=(ColorToRemove>>16)&0xFF;
               t[(x*3)+1]=(ColorToRemove>>8)&0xFF;
               t[(x*3)+2]=(ColorToRemove&0xFF);
            }
            else //if(t[(x*3)]==0xFF) //transparent WHITE
            {
               t[(x*3)]=(IColorToRemove>>16)&0xFF;
               t[(x*3)+1]=(IColorToRemove>>8)&0xFF;
               t[(x*3)+2]=(IColorToRemove&0xFF);
            }
